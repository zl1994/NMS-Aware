...
class SABLHead(nn.Module):
    ...
    def get_bboxes(self,
        ...
        if bbox_pred is not None:
            bboxes, confids, side_confids = self.bbox_coder.decode(rois[:, 1:], bbox_pred, img_shape)
        else:
            bboxes = rois[:, 1:].clone()
            confids = None
        ...
        if rescale and bboxes.size(0) > 0:
            if isinstance(scale_factor, float):
                ...
                gt_bboxes /= torch.from_numpy(scale_factor).to(gt_bboxes.device)
        
        if gt_bboxes.shape[0]!=0:
            
            iou = bbox_overlaps(rois[:, 1:], gt_bboxes)
            val, inds = iou.max(dim=1)
            filter_inds = (val>0.5).nonzero(as_tuple=True)[0]
            target_gt_bboxes = gt_bboxes[inds]
            side_distance = torch.abs(rois[:, 1:]-target_gt_bboxes)
        ...
        if cfg is None:
            ...
            return det_bboxes, det_labels, side_distance, side_confids
