...
class CornerHead(BaseDenseHead):
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            result_list = self._get_bboxes_single(
                    ...
                    rescale=rescale,
                    with_nms=with_nms, gt_bboxes=gt_bboxes[img_id][0], 
                    gt_labels=gt_labels[img_id][0],
                    dense_bboxes=dense_bboxes)
            '''
            result_list.append(
    ...
    def _get_bboxes_single(self,
        ...
        batch_bboxes, batch_scores, batch_clses, tl_scores, br_scores, tl_off, br_off = self.decode_heatmap(
        ...
        if rescale:
            ...
            gt_bboxes /= gt_bboxes.new_tensor(img_meta['scale_factor'])
        ...
        clses = batch_clses.view([-1, 1])
         ...
         #-----------------------------------------------------------------------------
        # load dense_bboxes
        dense_bboxes = dense_bboxes.to(device=gt_bboxes.device)
        dense_bboxes = dense_bboxes.reshape(-1, 4)
         #-----------------------------------------------------------------------------
        if gt_bboxes.shape[0]!=0:
            iou = bbox_overlaps(dense_bboxes, gt_bboxes)
            val, inds = iou.max(dim=1)
            filter_inds = (val>0.5).nonzero(as_tuple=True)
            target_gt_bboxes = gt_bboxes[inds]
            delta_tl_x = torch.abs(target_gt_bboxes[..., 0]-dense_bboxes[..., 0])
            delta_br_x = torch.abs(target_gt_bboxes[..., 2]-dense_bboxes[..., 2])
            delta_tl_y = torch.abs(target_gt_bboxes[..., 1]-dense_bboxes[..., 1])
            delta_br_y = torch.abs(target_gt_bboxes[..., 3]-dense_bboxes[..., 3])
            '''
            iou = bbox_overlaps(bboxes, gt_bboxes)
            val, inds = iou.max(dim=1)
            filter_inds = (val>0.5).nonzero(as_tuple=True)
            target_gt_bboxes = gt_bboxes[inds]
            delta_tl_x = torch.abs(target_gt_bboxes[..., 0]-bboxes[..., 0])
            delta_br_x = torch.abs(target_gt_bboxes[..., 2]-bboxes[..., 2])
            delta_tl_y = torch.abs(target_gt_bboxes[..., 1]-bboxes[..., 1])
            delta_br_y = torch.abs(target_gt_bboxes[..., 3]-bboxes[..., 3])
            tl_scores = tl_scores[0]
            br_scores = br_scores[0]
            
            delta_tl_x = delta_tl_x[filter_inds]
            delta_br_x = delta_br_x[filter_inds]
            delta_tl_y = delta_tl_y[filter_inds]
            delta_br_y = delta_br_y[filter_inds]
            tl_scores  = tl_scores[filter_inds]
            br_scores  = br_scores[filter_inds]
            '''
        else:
            delta_tl_x = None
            delta_br_x = None
            ...
            delta_br_y = None
        #-----------------------------------------------------------------------------
        # get tl_scores and br_scores
        tl_heat=tl_heat.sigmoid()
        br_heat=br_heat.sigmoid()

    
        output = tl_heat.permute(0, 2, 3, 1)
        output,_ = output.max(dim=-1)
        br_output = br_heat.permute(0, 2, 3, 1)
        br_output,_ = br_output.max(dim=-1)
        ...
        dense_bboxes[:, 0] += x_off
        dense_bboxes[:, 1] += y_off
        dense_bboxes[:, 2] += x_off
        dense_bboxes[:, 3] += y_off

        dense_bboxes[:, 0] /= (inp_w / width)
        dense_bboxes[:, 1] /= (inp_h / height)
        dense_bboxes[:, 2] /= (inp_w / width)
        dense_bboxes[:, 3] /= (inp_h / height)
        
        dense_tl = dense_bboxes[:, :2]
        dense_br = dense_bboxes[:, 2:]
        output = output.unsqueeze(dim=0)
        br_output = br_output.unsqueeze(dim=0)
        h = output.shape[-2]
        w = output.shape[-1]
        dense_tl[:, 0] = (dense_tl[:, 0]-w/2)/w*2
        dense_tl[:, 1] = (dense_tl[:, 1]-h/2)/h*2
        dense_br[:, 0] = (dense_br[:, 0]-w/2)/w*2
        dense_br[:, 1] = (dense_br[:, 1]-h/2)/h*2

        dense_tl = dense_tl.unsqueeze(dim=0).unsqueeze(dim=0)
        dense_br = dense_br.unsqueeze(dim=0).unsqueeze(dim=0)
        
        tl_scores = torch.nn.functional.grid_sample(output, dense_tl, mode='bilinear', padding_mode='zeros', align_corners=True)
        tl_scores = tl_scores.reshape(-1)
        br_scores = torch.nn.functional.grid_sample(br_output, dense_br, mode='bilinear', padding_mode='zeros', align_corners=True)
        br_scores = br_scores.reshape(-1)

        tl_offs = torch.nn.functional.grid_sample(tl_off, dense_tl, mode='bilinear', padding_mode='zeros', align_corners=True)
        tl_offs = tl_offs.permute(0, 2, 3, 1)
        tl_offs = tl_offs.squeeze(dim=0).squeeze(dim=0)
        br_offs = torch.nn.functional.grid_sample(br_off, dense_br, mode='bilinear', padding_mode='zeros', align_corners=True)
        br_offs = br_offs.permute(0, 2, 3, 1)
        br_offs = br_offs.squeeze(dim=0).squeeze(dim=0)
        #-----------------------------------------------------------------------------
        ...
        dense_bboxes = torch.from_numpy(dense_bboxes).to(device=gt_bboxes.device)
        dense_bboxes = dense_bboxes.reshape(-1, 4)
        ...
        dense_tl[:, 1] = (dense_tl[:, 1]-h/2)/h*2
        dense_tl = dense_tl.unsqueeze(dim=0).unsqueeze(dim=0)
        sample_score = torch.nn.functional.grid_sample(output, dense_tl, mode='bilinear', padding_mode='zeros', align_corners=True)
        sample_score = sample_score.reshape(-1)
        ...
        gt_bboxes[:, 3] += y_off
        '''
        pseudo_dense_scores = gt_bboxes.new_ones((dense_bboxes.shape[0], 1))
        dense_bboxes = torch.cat([dense_bboxes, pseudo_dense_scores], dim=1)
        '''
        pseudo_scores = gt_bboxes.new_ones((gt_bboxes.shape[0], 1))
        gt_detections = torch.cat([pseudo_scores], dim=1)
        gt_labels = gt_labels
        detections = torch.cat([detections], dim=0)
        labels = torch.cat([labels], dim=0)
        
        det_results = [tuple([detections, labels])]
        print(tl_offs.shape)
        return det_results, delta_tl_x, delta_tl_y, delta_br_x, delta_br_y, tl_scores, br_scores, tl_offs, br_offs
    ...
    def decode_heatmap(self,
        ...
        out_tl_off = tl_off*(inp_w / width)
        out_br_off = br_off*(inp_h / height)
        ...
        topk_tl_inds = (inds // 100).long()
        topk_br_inds = (inds % 100).long()
        tl_scores = tl_scores[0, topk_tl_inds, 0]
        br_scores = br_scores[0, 0, topk_br_inds]
        ...
        return bboxes, scores, clses, tl_scores, br_scores, out_tl_off, out_br_off
