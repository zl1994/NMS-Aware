...
class VFNetHead(ATSSHead, FCOSHead):
    ...
    def _init_layers(self):
        ...
        '''
        self.vfnet_reg_refine_dconv = ConvModule(
                    self.feat_channels,
                    self.feat_channels,
                    3,
                    stride=1,
        ...
        '''
        self.vfnet_reg_refine_x1 = nn.Conv2d(self.feat_channels, 1, 1)
        self.vfnet_reg_refine_y1 = nn.Conv2d(self.feat_channels, 1, 1)
        self.vfnet_reg_refine_x2 = nn.Conv2d(self.feat_channels, 1, 1)
        self.vfnet_reg_refine_y2 = nn.Conv2d(self.feat_channels, 1, 1)
        '''
    ...
    def forward_single(self, x, scale, scale_refine, stride, reg_denom):
        ...
        '''
        batch, C, H, W = reg_feat.shape
        conv_feat = reg_feat.reshape(-1, self.in_channels)
        residual_feat = self.instance_interactive_conv(conv_feat, reg_feat)
        conv_feat = conv_feat + self.instance_interactive_conv_dropout(residual_feat)
        reg_feat = self.instance_interactive_conv_norm(conv_feat)
        print(reg_feat.shape)
        reg_feat = reg_feat.reshape(batch, -1, H, W)
        print('after:', reg_feat.shape)
        '''
        #reg_feat_x1 = self.deform_sampling(reg_feat.contiguous(), dcn_offset[:,:2,:,:].contiguous())
        #print(reg_feat_x1.shape)
        reg_feat = self.relu(self.vfnet_reg_refine_dconv(reg_feat, dcn_offset))
        #reg_feat = self.vfnet_reg_refine_dconv(reg_feat)
        bbox_pred_refine = scale_refine(self.vfnet_reg_refine(reg_feat)).float().exp()
        #bbox_pred_refine = self.vfnet_reg_refine(reg_feat)
    ...
    def deform_sampling(self, feat, offset):
        ...
        b, c, h, w = feat.shape
        weight = feat.new_ones(c, c, 1, 1)
        y = deform_conv2d(feat, offset, weight, 1, 0, 1)
        return y
    ...
    def loss(self,
        ...
        if num_pos > 0:
            ...
            w = (pos_decoded_bbox_preds[..., 2]-pos_decoded_bbox_preds[..., 0]).clamp(min=1e-6)
            h = (pos_decoded_bbox_preds[..., 3]-pos_decoded_bbox_preds[..., 1]).clamp(min=1e-6)
            #side_distance = (torch.abs(pos_decoded_bbox_preds-pos_decoded_target_preds)/24.0).clamp(max=2)
            side_distance = torch.abs(pos_decoded_bbox_preds-pos_decoded_target_preds)
            side_distance[..., 0] = (side_distance[..., 0]*10/w).clamp(max=2)
            side_distance[..., 1] = (side_distance[..., 1]*10/h).clamp(max=2)
            side_distance[..., 2] = (side_distance[..., 2]*10/w).clamp(max=2)
            side_distance[..., 3] = (side_distance[..., 3]*10/h).clamp(max=2)
            
            loss_bbox_refine = self.loss_bbox_refine(
                pos_bbox_preds_refine,
                side_distance.detach(),
                avg_factor=num_pos_avg_per_gpu)
            iou_targets_rf = iou_targets_ini
            '''
            gt_scale = bbox2distance(pos_points, pos_decoded_target_preds)
            gt_scale = gt_scale/pos_bbox_preds.detach()
            print(gt_scale)
            loss_bbox_refine = self.loss_bbox_refine(
                pos_bbox_preds_refine,
                gt_scale.detach(),
            ...
            pos_decoded_bbox_preds_refine = distance2bbox(pos_points, pos_bbox_preds_refine*pos_bbox_preds)            
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            result_list = self._get_bboxes_single(cls_score_list,
                                                 bbox_pred_list, bbox_pred_refine_list, mlvl_points,
                                                 img_shape, scale_factor, cfg,
                                                 rescale, with_nms, gt_bboxes=gt_bboxes[img_id][0], 
    ...
    def _get_bboxes_single(self,
        ...
        mlvl_point = []
        mlvl_bbox_pred = []
        mlvl_bbox_pred_refine = []
        for cls_score, bbox_pred, bbox_pred_refine, points in zip(cls_scores, bbox_preds, bbox_pred_refines,
                                                mlvl_points):
            ...
            bbox_pred_refine = bbox_pred_refine.permute(1, 2, 0).reshape(-1, 4).contiguous()
            ...
            if 0 < nms_pre < scores.shape[0]:
                ...
                bbox_pred_refine = bbox_pred_refine[topk_inds, :]
                scores = scores[topk_inds, :]
            bboxes = distance2bbox(points, bbox_pred, max_shape=img_shape)
            refine_bboxes = distance2bbox(points, bbox_pred*bbox_pred_refine, max_shape=img_shape)
            mlvl_point.append(points)
            mlvl_bboxes.append(bboxes)
            mlvl_refine_bboxes.append(refine_bboxes)
            mlvl_scores.append(scores)
            mlvl_bbox_pred.append(bbox_pred)
            mlvl_bbox_pred_refine.append(bbox_pred_refine)
        mlvl_bboxes = torch.cat(mlvl_bboxes)
        mlvl_refine_bboxes = torch.cat(mlvl_refine_bboxes)
        if rescale:
            mlvl_bboxes /= mlvl_bboxes.new_tensor(scale_factor)
            mlvl_refine_bboxes /= mlvl_refine_bboxes.new_tensor(scale_factor)
            gt_bboxes /= gt_bboxes.new_tensor(scale_factor)
        
        if gt_bboxes.shape[0]!=0:
            mlvl_point = torch.cat(mlvl_point)
            mlvl_bbox_pred = torch.cat(mlvl_bbox_pred)
            mlvl_bbox_pred_refine = torch.cat(mlvl_bbox_pred_refine)

            iou = bbox_overlaps(mlvl_bboxes, gt_bboxes)
            val, inds = iou.max(dim=1)

            refine_iou = bbox_overlaps(mlvl_refine_bboxes, gt_bboxes)
            refine_val, _ = refine_iou.max(dim=1)
            imp_iou = refine_val-val
            
            target_gt_bboxes = gt_bboxes[inds]
            gt_scale = bbox2distance(mlvl_point, target_gt_bboxes)
            gt_scale = gt_scale/mlvl_bbox_pred.detach()
            side_distance = gt_scale
            
            mask = (side_distance<15) & (side_distance>0)
            inds = (mask.all(dim=1)).nonzero(as_tuple=True)[0]
            inds = (val>0.5).nonzero(as_tuple=True)[0]
            side_distance = side_distance[inds]
            mlvl_bbox_pred_refine = mlvl_bbox_pred_refine[inds]
            imp_iou=imp_iou[inds]
            '''
            side_distance = torch.abs(mlvl_bboxes-target_gt_bboxes)
            mlvl_bbox_pred_refine = torch.cat(mlvl_bbox_pred_refine)

            w = (mlvl_bboxes[..., 2]-mlvl_bboxes[..., 0]).clamp(min=1e-6)
            h = (mlvl_bboxes[..., 3]-mlvl_bboxes[..., 1]).clamp(min=1e-6)
            mlvl_bbox_pred_refine[..., 0] = mlvl_bbox_pred_refine[..., 0]*w/10
            mlvl_bbox_pred_refine[..., 1] = mlvl_bbox_pred_refine[..., 1]*h/10
            mlvl_bbox_pred_refine[..., 2] = mlvl_bbox_pred_refine[..., 2]*w/10
            mlvl_bbox_pred_refine[..., 3] = mlvl_bbox_pred_refine[..., 3]*h/10
            mask = side_distance<20
            inds = (mask.all(dim=1)).nonzero(as_tuple=True)[0]
            side_distance = side_distance[inds]
            mlvl_bbox_pred_refine = mlvl_bbox_pred_refine[inds]
            '''
        ...
        if with_nms:
            det_bboxes, det_labels = multiclass_nms(mlvl_bboxes, mlvl_scores,
                                                    ...
                                                    cfg.max_per_img)
            det_results = [tuple([det_bboxes, det_labels])]
            return det_results, side_distance, mlvl_bbox_pred_refine, imp_iou
        else:
